...
if len(sys.argv) < 3:
    ...
    # hashmap that records ORF IDs (keys) and annotation IDs (values)
    annotations = {}

    for line in overlap:
        line = line.rstrip()
        elems = line.split("\t")
        # remember the last functional annotation for an ORF that exists in file
        annotations[elems[4]] = elems[8]
    ...
    for line in ORFPairs:
        ...
        print("\t".join([line, ",".join(listAnnot),
              str(countAnnot), str(annotPercentage)]))
