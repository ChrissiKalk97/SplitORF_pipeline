...
```{r hist1, echo=FALSE}
ggplot(tab, aes(x=NumOrfs)) + geom_histogram(binwidth=0.1) +
  geom_textvline(label = paste("mean number of ORFs", round(mean(tab$NumOrfs), 3)), xintercept = mean(tab$NumOrfs), vjust = 1.3)+
...
```{r hist2, echo=FALSE}
require(dplyr)
table(tab$geneID) %>% as.data.frame %>% ggplot(aes(x = Freq)) + geom_histogram(binwidth=0.5) +
  geom_textvline(label = paste("mean number of t - p pairs per gene", round(mean(table(tab$geneID)), 3)), xintercept = mean(table(tab$geneID)), vjust = 1.3)+
...
require(gridExtra)
plot1 <- qplot(as.numeric(unlist(strsplit(as.character(tab$OrfLengths),",",fixed=T))))+
  labs(title="Nucleotide lengths of all ORFs", x="ORF length", y="Frequency")
plot2 <- qplot(sapply(strsplit(as.character(tab$OrfLengths),",",fixed=T),function(x){return(mean(as.numeric(x)))}))+
  labs(title="Mean ORF lengths per SplitORF transcript", x="ORF length", y="Frequency")
...
require(gridExtra)
plot1 <- qplot(as.numeric(unlist(strsplit(as.character(tab$OrfLengths),",",fixed=T))), xlim = c(0,2000))+
  labs(title="Nucleotide lengths of all ORFs", x="ORF length", y="Frequency")
plot2 <- qplot(sapply(strsplit(as.character(tab$OrfLengths),",",fixed=T),function(x){return(mean(as.numeric(x)))}), xlim = c(0,2000))+
  labs(title="Mean ORF lengths per SplitORF transcript", x="ORF length", y="Frequency")
grid.arrange(plot1, plot2, ncol=2)
```
```{r, echo=FALSE}
m <- matrix(c(round(mean(as.numeric(unlist(strsplit(as.character(tab$OrfLengths),",",fixed=T), 5)))), mean(sapply(strsplit(as.character(tab$OrfLengths),",",fixed=T),function(x){return(mean(as.numeric(x)))}))), ncol = 2, nrow = 1)
mean_ORF_lengths <- data.frame(m)
rownames(mean_ORF_lengths) = "mean ORF length"
colnames(mean_ORF_lengths) = c("mean overall ORF length", "mean ORF length per gene")
...
rownames(mean_ORF_lengths) = "mean ORF length in AA"
colnames(mean_ORF_lengths) = c("mean overall ORF length in AA", "mean ORF length per gene in AA")
print(mean_ORF_lengths)
```

```{r orfLengthsAA, echo=FALSE}
require(gridExtra)
plot1 <- qplot(as.numeric(unlist(strsplit(as.character(tab$OrfLengths),",",fixed=T)))/3, xlim = c(0,2000))+
  labs(title="AA lengths of all ORFs", x="ORF length", y="Frequency")
plot2 <- qplot(sapply(strsplit(as.character(tab$OrfLengths),",",fixed=T),function(x){return(mean(as.numeric(x))/3)}), xlim = c(0,2000))+
  labs(title="Mean AA ORF lengths per SplitORF transcript", x="ORF length", y="Frequency")
grid.arrange(plot1, plot2, ncol=2)
```
```{r orfLengthsAAzoom, echo=FALSE}
require(gridExtra)
plot1 <- qplot(as.numeric(unlist(strsplit(as.character(tab$OrfLengths),",",fixed=T)))/3, xlim = c(0,1000))+
  labs(title="AA lengths of all ORFs", x="ORF length", y="Frequency")
plot2 <- qplot(sapply(strsplit(as.character(tab$OrfLengths),",",fixed=T),function(x){return(mean(as.numeric(x))/3)}), xlim = c(0,1000))+
  labs(title="Mean AA ORF lengths per SplitORF transcript", x="ORF length", y="Frequency")
grid.arrange(plot1, plot2, ncol=2)
```
...
```{r orfAnnots, echo=FALSE}
 anno_present <- tab %>% subset(AnnotPercent >0) 
    ggplot(anno_present, aes(x = NumORFAnnot)) + xlim(0, 6)+
  geom_histogram() + labs(title="Frequency of ORFs with >= 1 annotation", x="Number of ORFs that are overlapping annotation", y="Frequency")
...
``` {r stats annotations Gene, echo=FALSE}
    ...
    domain_counts <- subTab %>% group_by(ORF.DomainAnnot) %>% count()
...
``` {r stats annotations, echo=FALSE}
    domain_counts_all <- tab %>% group_by(ORF.DomainAnnot) %>% count()
